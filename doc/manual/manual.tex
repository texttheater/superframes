...
\begin{table}
    \resizebox{\textwidth}{!}{
        \begin{NiceTabular}{lllllll}
            ...
            \fr{EVENTUALITY} & & & & & & \ref{sec:SUPERFRAME} \\

            \Sub\fr{PREDICATION} & & \rl{argument} & \rl{predicate} & & & \ref{sec:PREDICATION} \\

            \SubSub\fr{ACTIVITY} & & \rl{is-active} & \rl{activity} & & & \ref{sec:ACTIVITY} \\

            \SubSub\fr{CLASS} & \rl{initial-class} & \rl{has-class} & \rl{class} & & \rl{target-class} & \ref{sec:CLASS} \\

            \SubSub\fr{EXISTENCE} & & & \rl{exists} & & & \ref{sec:EXISTENCE} \\
            \SubSubSub\fr{TRANSFORMATION-CREATION} & & \rl{material} & & & \rl{created} & \ref{sec:TRANSFORMATION-CREATION} \\
            \SubSubSub\fr{REPRODUCTION} & & \rl{original} & & & \rl{copy} & \ref{sec:REPRODUCTION} \\
