...
\begin{table}
    \resizebox{\textwidth}{!}{
        \begin{NiceTabular}{llllllr}
            ...
            \emoji{red-square} \fr{EVENTUALITY} & & & & & & \ref{sec:EVENTUALITY} \\

            \Sub\emoji{label} \fr{PREDICATION} & & \rl{argument} & \rl{predicate} & & & \ref{sec:PREDICATION} \\

            \SubSub\emoji{woman-dancing} \fr{ACTIVITY} & & \rl{is-active} & \rl{activity} & & & \ref{sec:ACTIVITY} \\

            \SubSub\emoji{deciduous-tree} \fr{CLASS} & \rl{initial-class} & \rl{has-class} & \rl{class} & & \rl{target-class} & \ref{sec:CLASS} \\

            \SubSub\emoji{sparkles} \fr{EXISTENCE} & & & \rl{exists} & & & \ref{sec:EXISTENCE} \\
            \SubSubSub\emoji{memo} \fr{REPRODUCTION} & & \rl{original} & & & \rl{copy} & \ref{sec:REPRODUCTION} \\
            \SubSubSub\emoji{magic-wand} \fr{TRANSFORMATION-CREATION} & & \rl{material} & & & \rl{created} & \ref{sec:TRANSFORMATION-CREATION} \\

            \SubSub\emoji{eye} \fr{EXPERIENCE} & \rl{initial-experience} & \rl{experiencer} & \rl{experience} & \rl{transitory-experience} & \rl{target-experience} & \ref{sec:EXPERIENCE} \\

            \SubSub\emoji{passport-control} \fr{IDENTIFICATION} & & \rl{identified} & \rl{identifier} & & & \ref{sec:IDENTIFICATION} \\

            \SubSub\emoji{red-question-mark} \fr{MODE} & & \rl{has-mode} & \rl{mode} & & & \ref{sec:MODE} \\

            \SubSub\emoji{red-apple} \fr{QUALITY} & & \rl{has-quality} & \rl{quality} & & & \ref{sec:QUALITY} \\

            \SubSub\emoji{abacus} \fr{QUANTITY} & & \rl{has-quantity} & \rl{quantity} & & & \ref{sec:QUANTITY} \\

            \SubSub\emoji{1st-place-medal} \fr{RANK} & & \rl{has-rank} & \rl{rank} & & & \ref{sec:RANK} \\

            \SubSub\emoji{zzz} \fr{STATE} & \rl{initial-state} & \rl{has-state} & \rl{state} & & \rl{target-state} & \ref{sec:STATE} \\
            \SubSubSub\emoji{skull} \fr{DESTRUCTION} & & \rl{destroyed} & & & & \ref{sec:DESTRUCTION} \\

            \Sub\emoji{link} \fr{RELATION} & & \rl{satellite} & \rl{nucleus} & & & \ref{sec:RELATION} \\

            \SubSub\emoji{fork-and-knife} \fr{ACCOMPANIMENT} & & \rl{accompanied} & \rl{accompanier} & & & \ref{sec:ACCOMPANIMENT} \\
            \SubSubSub\emoji{paintbrush} \fr{DEPICTIVE} & & \rl{has-depictive} & \rl{depictive} & & & \ref{sec:DEPICTIVE} \\

            \SubSub\emoji{money-bag} \fr{ASSET} & & \rl{has-asset} & \rl{asset} & & & \ref{sec:ASSET} \\

            \SubSub\emoji{keycap-asterisk} \fr{ATTRIBUTE} & & \rl{has-attribute} & \rl{attribute} & & & \ref{sec:ATTRIBUTE} \\

            \SubSub\emoji{balance-scale} \fr{COMPARISON} & & \rl{compared} & \rl{reference} & & & \ref{sec:COMPARISON} \\
            \SubSubSub\emoji{ok-hand} \fr{CONCESSION} & & \rl{assertion} & \rl{conceded} & & & \ref{sec:CONCESSION} \\

            \SubSub\emoji{red-exclamation-mark} \fr{EXPLANATION} & & \rl{explained} & \rl{explanation} & & & \ref{sec:EXPLANATION} \\
            \SubSubSub\emoji{bullseye} \fr{PURPOSE} & & \rl{has-purpoe} & \rl{purpose} & & & \ref{sec:PURPOSE} \\

            \SubSub\emoji{round-pushpin} \fr{LOCATION} & \rl{initial-location} & \rl{has-location} & \rl{location} & \rl{transitory-location} & \rl{target-location} & \ref{sec:LOCATION} \\
            \SubSubSub\emoji{gem-stone} \fr{ADORNMENT-TARNISHMENT} & \rl{initial-surface} & \rl{ornament} & \rl{surface} & & \rl{target-surface} & \ref{sec:ADORNMENT-TARNISHMENT} \\
            \SubSubSub\emoji{sweat-droplets} \fr{EXCRETION} & \rl{excreter} & \rl{excreted} & & \rl{transitory-location} & \rl{target-location} & \ref{sec:EXCRETION} \\
            \SubSubSub\emoji{cricket-game} \fr{HITTING} & & \rl{hitting} & \rl{hit} & & & \ref{sec:HITTING} \\
            \SubSubSub\emoji{bowl-with-spoon} \fr{INGESTION} & & \rl{ingested} & & \rl{transitory-location} & \rl{ingester} & \ref{sec:INGESTION} \\
            \SubSubSub\emoji{leaf-fluttering-in-wind} \fr{UNANCHORED-MOTION} & & \rl{in-motion} & & \rl{transitory-location} & & \ref{sec:UNANCHORED-MOTION} \\
            \SubSubSub\emoji{t-shirt} \fr{WRAPPING-WEARING} & & \rl{worn} & \rl{wearer} & & & \ref{sec:WRAPPING-WEARING} \\

            \SubSub\emoji{hammer} \fr{MEANS} & & \rl{has-means} & \rl{means} & & & \ref{sec:MEANS} \\

            \SubSub\emoji{left-speech-bubble} \fr{MESSAGE} & & \rl{topic} & \rl{content} & & & \ref{sec:MESSAGE} \\

            \SubSub\emoji{spider-web} \fr{NONCOMP} & & \rl{has-noncomp} & \rl{noncomp} & & & \ref{sec:NONCOMP} \\

            \SubSub\emoji{puzzle-piece} \fr{PART-WHOLE} & \rl{initial-whole} & \rl{part} & \rl{whole} & & \rl{target-whole} & \ref{sec:PART-WHOLE} \\

            \SubSub\emoji{service-dog} \fr{POSSESSION} & \rl{initial-possessor} & \rl{possessed} & \rl{possessor} & & \rl{target-possessor} & \ref{sec:POSSESSION} \\

            \SubSub\emoji{performing-arts} \fr{SCENE} & \rl{initial-scene} & \rl{participant} & \rl{scene} & \rl{transitory-scene} & \rl{target-scene} & \ref{sec:SCENE} \\

            \SubSub\emoji{megaphone} \fr{SENDING} & & \rl{sent} & \rl{sender} & & & \ref{sec:SENDING} \\

            \SubSub\emoji{right-arrow} \fr{SEQUENCE} & & \rl{follows} & \rl{followed} & & & \ref{sec:SEQUENCE} \\
            \SubSubSub\emoji{joystick} \fr{CAUSATION} & & \rl{result} & \rl{causer} & & & \ref{sec:CAUSATION} \\
            \SubSubSub\emoji{scroll} \fr{CONDITION} & & \rl{has-condition} & \rl{condition} & & & \ref{sec:CONDITION} \\
            \SubSubSub\emoji{prohibited} \fr{EXCEPTION} & & \rl{has-exception} & \rl{exception} & & & \ref{sec:EXCEPTION} \\
            \SubSubSub\emoji{collision} \fr{REACTION} & & \rl{reaction} & \rl{trigger} & & & \ref{sec:REACTION} \\
            \SubSubSub\emoji{sneezing-face} \fr{RESULTATIVE} & & \rl{has-resultative} & \rl{resultative} & & & \ref{sec:RESULTATIVE} \\

            \SubSub\emoji{handshake} \fr{SOCIAL-RELATION} & \rl{initial-social-relation} & \rl{has-social-relation} & \rl{social-relation} & & \rl{target-social-relation} & \ref{sec:SOCIAL-RELATION} \\

            \SubSub\emoji{alarm-clock} \fr{TIME} & & \rl{has-time} & \rl{time} & & & \ref{sec:TIME} \\
...
\subsection{Aspect, Mode, and Polarity}
...
\ex.\dep{Kim_x-causer *saved#EXPERIENCE-PREVENTION* Sandy_experiencer from the dragon_target-experience}

In the last example, \emph{dragon} is to be understood metonymically as an
experience in which Sandy would have been harmed by the dragon.
...
\subsection{\emoji{red-square} \fr{EVENTUALITY}}
...
\ex.\dep{*Yes#EVENTUALITY*}

\ex.\dep{*No#EVENTUALITY-NEG*}

\ex.\dep{*What#IDENTIFICATION>>EVENTUALITY* ?}

\subsection{\emoji{label} \fr{PREDICATION}}
...
\subsection{\emoji{woman-dancing} \fr{ACTIVITY}}
\label{sec:ACTIVITY}
...
\subsection{\emoji{deciduous-tree} \fr{CLASS}}
\label{sec:CLASS}
...
\subsection{\emoji{sparkles} \fr{EXISTENCE}}
\label{sec:EXISTENCE}
...
\subsection{\emoji{memo} \fr{REPRODUCTION}}
...
\subsection{\emoji{magic-wand} \fr{TRANSFORMATION-CREATION}}
...
\subsection{\emoji{eye} \fr{EXPERIENCE}}
...
\subsection{\emoji{passport-control} \fr{IDENTIFICATION}}
...
\subsection{\emoji{red-question-mark} \fr{MODE}}
\label{sec:MODE}
...
\subsection{\emoji{red-apple} \fr{QUALITY}}
\label{sec:QUALITY}
...
\subsection{\emoji{abacus} \fr{QUANTITY}}
\label{sec:QUANTITY}
...
\subsection{\emoji{1st-place-medal} \fr{RANK}}
\label{sec:RANK}
...
\subsection{\emoji{skull} \fr{DESTRUCTION}}
...
\subsection{\emoji{link} \fr{RELATION}}
\label{sec:RELATION}
...
\subsection{\emoji{fork-and-knife} \fr{ACCOMPANIMENT}}
...
\subsection{\emoji{paintbrush} \fr{DEPICTIVE}}
\label{sec:DEPICTIVE}
...
\subsection{\emoji{money-bag} \fr{ASSET}}
\label{sec:ASSET}
...
\subsection{\emoji{keycap-asterisk} \fr{ATTRIBUTE}}
\label{sec:ATTRIBUTE}
...
\subsection{\emoji{balance-scale} \fr{COMPARISON}}
...
\subsection{\emoji{ok-hand} \fr{CONCESSION}}
...
\subsection{\emoji{red-exclamation-mark} \fr{EXPLANATION}}
...
\subsection{\emoji{bullseye} \fr{PURPOSE}}
\label{sec:PURPOSE}
...
\subsection{\emoji{round-pushpin} \fr{LOCATION}}
\label{sec:LOCATION}
...
\subsection{\emoji{gem-stone} \fr{ADORNMENT-TARNISHMENT}}
...
\subsection{\emoji{sweat-droplets} \fr{EXCRETION}}
\label{sec:EXCRETION}
...
\subsection{\emoji{cricket-game} \fr{HITTING}}
\label{sec:HITTING}
...
\subsection{\emoji{bowl-with-spoon} \fr{INGESTION}}
\label{sec:INGESTION}
...
\subsection{\emoji{leaf-fluttering-in-wind} \fr{UNANCHORED-MOTION}}
...
\subsection{\emoji{t-shirt} \fr{WRAPPING-WEARING}}
...
\subsection{\emoji{hammer} \fr{MEANS}}
\label{sec:MEANS}
...
\subsection{\emoji{left-speech-bubble} \fr{MESSAGE}}
\label{sec:MESSAGE}

A message about \rl{topic} with content \rl{content} is received or exists in
recorded form. When a message is created through expression or observation, use
\fr{MESSAGE-INIT}. When \rl{content} and \rl{topic} are both realized,
\rl{content} must assign a role to \rl{topic}.
...
\subsubsection{Beginning and Ending Perception}

Use \fr{MESSAGE-INIT} (\fr{MESSAGE-DEINIT}, \fr{MESSAGE-PREVENTION}) for
predicates denoting the coming about (ending, failing to come about) of
...
\subsection{\emoji{spider-web} \fr{NONCOMP}}
\label{sec:NONCOMP}
...
\subsection{\emoji{puzzle-piece} \fr{PART-WHOLE}}
...
\subsection{\emoji{service-dog} \fr{POSSESSION}}
...
\subsection{\emoji{performing-arts} \fr{SCENE}}
\label{sec:SCENE}
...
\ex.\dep{Swift action_x-causer *prevented#SCENE-PREVENTION* an **outbreak#SCENE-INIT**_target-scene of ***measles#EXPERIENCE***__target-scene}
...
\subsection{\emoji{megaphone} \fr{SENDING}}
\label{sec:SENDING}
...
\subsection{\emoji{right-arrow} \fr{SEQUENCE}}
\label{sec:SEQUENCE}
...
\subsection{\emoji{joystick} \fr{CAUSATION}}
\label{sec:CAUSATION}
...
\subsection{\emoji{scroll} \fr{CONDITION}}
\label{sec:CONDITION}
...
\subsection{\emoji{prohibited} \fr{EXCEPTION}}
\label{sec:EXCEPTION}
...
\subsection{\emoji{collision} \fr{REACTION}}
\label{sec:REACTION}
...
\subsection{\emoji{sneezing-face} \fr{RESULTATIVE}}
...
\subsection{\emoji{handshake} \fr{SOCIAL-RELATION}}
...
\subsection{\emoji{alarm-clock} \fr{TIME}}
\label{sec:TIME}
...
\section{Argument Structure and Frame Choice}
...
\ex.
\a.\dep{Kim_x-causer *worked#EXPERIENCE*}
\b.\dep{Kim_x-causer *worked#EXPERIENCE* on the drawing_experiencer}
...
\subsection{However, Different Senses of a Predicate Can Have Different Arguments and Therefore Different Superframes}
...
\subsection{Look Up Unfamiliar Words in a Dictionary}

When you come across an unfamilar predicate, you might not be able to determine
what arguments it has, and consquenlty what the most appropriate superframe is,
...
\subsection{Symmetric Argument Pairs}
...
\subsection{When to Use \fr{SCENE}}
...
\ex.\dep{I_experiencer *lived#EXPERIENCE* my life_experience}

\ex.\dep{They_has-quantity *number#QUANTITY* in the thousands_quantity}

\section{Aspect, Mode, and Polarity}
...
\section{Construction-specific Guidelines}
...
\subsection{Pronouns with Arguments}
...
\subsection{Nominal Copula Constructions}
