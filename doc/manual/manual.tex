...
\begin{table}
    \resizebox{\textwidth}{!}{
        \begin{NiceTabular}{lllllll}
            ...
            \boro\fr{NONPARTICIPANT} & & \rl{has-nonparticipant} & \rl{nonparticipant} & & & \ref{sec:NONPARTICIPANT} \\
            \bottomrule
        \end{NiceTabular}
    }
...
acts as a light verb.  \rl{participant} is assigned a role by \rl{scene}. In
the following examples, we show the annotations for both the matrix predicate
and the embedded predicate in one graph.
...
\ex.\dep{Kim_x-causer>>x-sender *shook#UNANCHORED-MOTION>>MESSAGE* their head_has-location>>x-noncomp no_nonparticipant>>content}
...
\subsection{\fr{NONPARTICIPANT}}
\label{sec:NONPARTICIPANT}

\todo[inline]{move resultatives here}
...
Some nouns denote a person who participates in a specific type of scene in a specific type of role. In such cases, use the most appropriate frame for that scene. For example, in a narrative where the narrator has just been criticized by a stranger, you could annotate as follows:

\ex.\dep{With that, my_topic *critic#MESSAGE* sat down again}

In other cases, such nouns rather denote a person's profession or expertise or their role in a social context:
...
In fully non-decompositional particle verbs, where the meaning is not
