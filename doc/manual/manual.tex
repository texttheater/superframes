...
\begin{table}
    \resizebox{\textwidth}{!}{
        \begin{NiceTabular}{llllllr}
            ...
            \Sub\emoji{crossed-swords}\ \fr{EVENT} & & \rl{undergoer} & & & & \ref{sec:EVENT} \\
            \SubSub\emoji{woman-dancing} \fr{ACTIVITY} & & \rl{is-active} & & & & \ref{sec:ACTIVITY} \\
            ...
            \SubSub\emoji{t-shirt} \fr{WRAPPING-WEARING} & \rl{initial-wearer} & \rl{worn} & \rl{wearer} & & \rl{target-wearer} & \ref{sec:WRAPPING-WEARING} \\
            ...
            \Sub\emoji{red-question-mark} \fr{MODE} & & \rl{has-mode} & & & & \ref{sec:MODE} \\
            ...
            \Sub\emoji{zzz} \fr{STATE} & & \rl{has-state} & & & & \ref{sec:STATE} \\
            \SubSub\emoji{red-apple} \fr{QUALITY} & & \rl{has-quality} & & & & \ref{sec:QUALITY} \\
            \SubSubSub\emoji{deciduous-tree} \fr{CLASS} & & \rl{has-class} & & & & \ref{sec:CLASS} \\
...
\newpage\subsection{Reverse Roles}

There is another type of modifier that is syntactically a dependent, but
semantically a predicate of which the modifiee is an argument. Here, we label
the modifier with the role that the \emph{modifiee} fills, prefixed with
\rl{r-}.

\ex.\dep{Kim_excreter is not_r-has-mode *sweating#EXCRETION* profusely_r-has-quality}.

\todo[inline]{also use for other sentence adverbs, relative clauses, etc.}

\todo[inline]{change annotation to use reverse roles where appropriate, especially with m-time, m-quantity. Adverbs especially should be annotated with reverse roles in most cases.}
...
\ex.\dep{despicable_r-has-quality *me#IDENTIFICATION*}

\ex.\dep{the tired_r-has-state *dog#CLASS*}

Similarly for adverbs denoting, e.g, manner (\rl{quality}) or extent (\rl{quantity}):

\ex.\dep{Kim_has-location *ran#Motion* fast_r-has-quality}

\ex.\dep{Kim_has-location *ran#Motion* far_r-has-quantity}
...
\ex.\dep{primeval_r-has-time>>m-noncomp *forest#CLASS*}

\ex.\dep{colored_r-has-quality>>m-noncomp *pencil#CLASS*}
...
\ex.\dep{more_r-has-quantity *isolated#SOCIAL-RELATION* than a shipwrecked sailor_x-reference}
...
\ex.\dep{Ich_x-experiencer bin trotzdem_r-asserted *fröhlich#MESSAGE*}
...
\ex.\dep{I_x-experiencer *pondered#MESSAGE-INIT* deeply_r-has-quality}
...
\ex.\dep{Kim_in-motion is an avid_r-has-quality *unicyclist#UNANCHORED-MOTION*}
...
\ex.\dep{Even_r-has-mode *Kim#IDENTIFICATION* did n't know that}

\ex.\dep{They_x-causer only_r-has-mode *rinsed#ADORNMENT-TARNISHMENT-DEINIT* the dishes_initial-surface}

\ex.\dep{*Passt#COMPARISON* das_compared eh_r-has-mode ?}

\ex.\dep{Kim_x-experiencer probably_r-has-mode *knows#MESSAGE* that_content}

\ex.\dep{That_has-quality 's really_r-has-mode *great#QUALITY*}

\ex.\dep{Kim_has-location is not_r-has-mode *here#LOCATION*}
...
\ex.\dep{Sie_x-experiencer *lernen#MESSAGE-INIT*__example **beispielsweise#EXAMPLE**_r-example wissenschaftliche Methoden_target-content}
...
\ex.\dep{three_r-has-quantity *burgers#CLASS*}

\ex.\dep{three_r-has-quantity *liters#QUANTITY* of coke_has-quantity}

\ex.\dep{We_x-sender *discourage#MESSAGE-INIT* this_topic emphatically_r-has-quantity}
...
\ex.\dep{my_m-sender first_r-has-rank *drawing#MESSAGE*}
...
\ex.\dep{a magnificent_r-has-quality *picture#MESSAGE*}

\ex.\dep{I_x-experiencer *pondered#MESSAGE-INIT* deeply_r-has-quality over the adventures_topic of the jungle}

\ex.\dep{a skilled_r-has-quality *surgeon#CLASS*}

\ex.\dep{such_r-has-quality *knowledge#MESSAGE* is valuable}
...
\ex.\dep{Kim_excreter *sneezed#EXCRETION* twice_r-has-time}
...
\ex.
\a.\dep{my_m-sender first_r-has-rank *drawing#MESSAGE*}
\b.\dep{my_m-sender first_r-has-rank *drawing#MESSAGE* of a snake_topic}
...
\ex.\dep{Obama_social-relation special_r-has-quality>>m-noncomp *assistant#SOCIAL-RELATION*}
...
\ex.\dep{So_r-has-explanation I_x-causer *tossed#TRANSFORMATION-CREATION* off_m-noncomp this drawing_created}
...
\ex.\dep{You_has-location may *go#LOCATION-INIT* in_target-location now_r-has-time}
...
\ex.\dep{*eat#INGESTION* up_r-has-quantity the cookies_ingested} (implies \emph{eat} the cookies)

In fully non-compositional particle verbs, where the meaning is not
predictable, use \rl{m-noncomp}.

\ex.\dep{*do#EVENT* somebody_undergoer in_m-noncomp} (does not imply \emph{do} somebody)

\ex.\dep{Es_x-sender *stellte#MESSAGE* ein riesiges Gesicht_topic dar_m-noncomp} (does not imply \emph{stellen} something)
...
\ex.\dep{My drawing_x-has-class was not_r-has-mode a *picture#MESSAGE* of a hat_topic}
...
\c.\dep{*sleep#STATE* a thousand miles__m-quantity **from#LOCATION**_r-has-location any human habitation__location}
