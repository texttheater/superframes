...
\begin{table}
    \resizebox{\textwidth}{!}{
        \begin{NiceTabular}{llllllr}
            ...
            \Sub\emoji{hammer} \fr{MEANS} & & \rl{purpose} & \rl{means} & & & \ref{sec:MEANS} \\
            ...
            \Sub\emoji{red-triangle-pointed-up} \fr{SUBCLASS} & & \rl{subclass} & \rl{superclass} & \rl{} & \rl{} & \ref{sec:SUBCLASS} \\
...
\ex.\dep{Kim_excreter is *sweating#EXCRETION* in the sauna_m-location}

\ex.\dep{Wir_participant *machen#SCENE* das_scene wegen der Kinder_m-explanation}

Adverbial clauses work much the same way, the have a conjunction instead of an
adposition:

\ex.\dep{*gekommen#LOCATION-INIT* , um zu bleiben_m-purpose}

A slightly more subtle case is adpositional phrases that are multiword
expressions such as \emph{vor allem}. Here, the relation is not so much
indicated by the adposition as by the combination of the adposition and its
complement. We still follow the syntactic edge that points to the complement:

\ex.\dep{Es_x-identified sind vor allem_m-reference>>m-mode die *Frauen#CLASS*}

\ex.\dep{Die *Sendung#MESSAGE*__initial-scene war zu **Ende#SCENE-DEINIT**_m-scene}
...
\ex.\dep{Und **doch#CONCESSION**_m-scene *sah#MESSAGE*__asserted er_topic intelligent_content aus_m-noncomp}
...
\ex.\dep{**immer#SCENE-CONTINUATION**_m-scene *höher#LOCATION-INIT*__initial-scene}
...
\ex.
\a.\dep{They appreciated how_m-quality Kim_is-active *danced#ACTIVITY*}
\b.\dep{They wondered how_m-means Kim_causer *did#CAUSATION* it_result}
\c.\dep{I remembered **how#MESSAGE**_m-scene my studies_x-experiencer had *concentrated#MESSAGE*__content on geography_topic}
...
\ex.\dep{Kim_x-causer left after *trashing#STATE-CHANGE* the room_has-state} (non-obligatory control)

\ex.\dep{Kim_has-location has come to *stay#LOCATION-CONTINUATION*} (infinitive of purpose)

\ex.\dep{Kim_has-location seemed to *fly#UNANCHORED-MOTION*} (raising)

\ex.\dep{Kim_x-sender entered the room *singing#MESSAGE-INIT*} (seondary predicate)
...
\ex.\dep{the song_topic I_x-experiencer *like#MESSAGE*} (reduced relative clause)

\ex.\dep{the song_topic *liked#MESSAGE* by Kim_x-experiencer} (non-finite reduced relative clause)

\ex.\dep{students_has-location *living#LOCATION* on campus_location} (non-finite reduced relative clause)

\ex.\dep{eine Gestalt__has-quality , deren **Magerkeit#QUALITY**_topic durch den Trainingsanzug_x-sender noch_m-mode *betont#MESSAGE* wurde} (relative clause with complex extracted element)

\ex.\dep{Atmosphäre_target-situator , mit der_target-situator sie_x-causer sich_theme zu *umgeben#SITUATION-INIT* wusste} (relative clause with extraction across clause boundaries)
...
\subsubsection{Secondary Predicates}

Secondary predicates are modifiers that syntactically attach to a (primary)
predicate, but semantically predicate over one of the primary predicate's
arguments, or even something more deeply embedded. The semantic relation
between the primary and secondary predicate can be one of simple accompaniment
(depictive), result (resultative), or something else.

\ex.\dep{Kim__has-location_x-sender **entered#LOCATION-INIT** the room *singing#MESSAGE-INIT*__m-depictive}

\ex.\dep{You're__x-sender **talking#MESSAGE-INIT** me_has-state__x-resultative *silly#STATE*__x-resultative}

\ex.\dep{*Stop#SCENE-DEINIT* drawing_initial-scene sheep__has-quality___has-quality , whether **small#QUALITY**_m-condition or ***big#QUALITY***_m-condition}

\ex.\dep{Some people_participant__has-quality , though **short#QUALITY**_m-conceded , *reach#SCENE-INIT* amazing heights_target-scene}
...
\ex.\dep{Sie_x-causer>>participant__x-experiencer *streifte#UNANCHORED-MOTION>>SCENE* ihn_transitory-location>>participant__topic mit einem **Seitenblick#MESSAGE-INIT**_has-location>>scene}

\ex.\dep{Kim_x-sender>>participant *refused#MESSAGE-INIT>>SCENE* to eat_topic>>scene}

\ex.\dep{ein *Stück#PART-WHOLE>>QUANTITY* Schwarzbrot_whole>>has-quantity}
...
\ex.\dep{to *lay#LOCATION-CHANGE>>MESSAGE-DEINIT* aside_target-location>>m-noncomp my drawings_has-location>>topic}

\ex.\dep{elektrischer_m-quality>>m-noncomp *Strom#CLASS*}
...
\ex.\dep{Winston_x-causer>>has-location *drehte#LOCATION-CHANGE* sich_has-location um_transitory-location}
...
\ex.\dep{Alice_x-causer *mixed#ACCOMPANIMENT-INIT* the egg_accompanied with the cream_target-accompanier}

\ex.\dep{Er_accompanier wurde mit den ihn umgebenden Menschen_accompanied *eins#ACCOMPANIMENT*}
...
Similarly for comparative verb alternation constructions:

\ex.\dep{Kim_has-rank *outranks#RANK* Sandy_x-reference}

\ex.\dep{Kim_x-sender *outshines#MESSAGE* Sandy_x-reference}

\ex.\dep{Sie_x-sender versuchten , die Stimme_x-reference zu *übertönen#MESSAGE-INIT*}
...
\ex.\dep{man_material wurde gegen seinen Willen_m-conceded in einen Verrückten_created *verwandelt#TRANSFORMATION-CREATION*}
...
\ex.\dep{Die Hassovation_initial-scene *steigerte#SCENE-CHANGE* sich zur Raserei_target-scene}
...
\rl{explanation} causes \rl{has-explanation} to be known or supposed.

\ex.\dep{She must be a *gardener#CLASS* , because he had seen_m-explanation her with a spade}
...
\ex.\dep{das *Abzeichen#IDENTIFICATION* der Jugendliga_identified gegen Sexualität}
...
Describes \rl{has-location} as located or moving with respect to \rl{location}.
...
Also used for orientations:

\ex.\dep{Die Nadel_has-location *zeigte#LOCATION* nach Norden_location}

\ex.\dep{Er_has-location *wandte#LOCATION* dem Televisor_location den Rücken_x-attribute zu}

Adverbs of location evoke \fr{LOCATION}:

\ex.\dep{Kim_has-location ist *oben#LOCATION*}

Adverbs of direction evoke \fr{LOCATION-INIT}:

\ex.\dep{Kim_has-location__has-location **fliegt#LOCATION-INIT** *hoch#LOCATION-INIT*__target-location}
...
\rl{means} is an intermediary causer of or is destined to serve a \rl{purpose}.
...
\ex.\dep{Kim_x-causer__x-causer *used#MEANS* a pen_means to **get#LOCATION-DEINIT**_purpose the lid__has-location off__initial-location}

\ex.\dep{You_x-causer *used#MEANS* me_means !}

\ex.\dep{oil_m-means *lamp#CLASS*}

\ex.\dep{die Nische_means war für ein Bücherregal_purpose *bestimmt#MEANS*}

\ex.\dep{Kim_has-location__target-possessor *went#LOCATION-CHANGE* to town_target-location to **buy#POSSESSION-CHANGE**_m-purpose food__possessed}
...
\ex.\dep{The jury_x-experiencer *found#MESSAGE-INIT* Kim_topic__participant___x-causer **guilty#SCENE**_target-content of the ***crime#EVENT***__scene}
...
Used for adverbial modifiers that provide information not about the scene
itself, but about the relationship between the linguistic expression and the
truth as presented -- for example, modal strength, precision,
literal/figurative use, etc.
...
\ex.\dep{They_x-sender literally_m-mode *screamed#MESSAGE-INIT*}

\ex.\dep{He_destroyed *kicked#DESTRUCTION* the proverbial_m-mode bucket_x-noncomp}

\ex.\dep{They walked for precisely_m-mode one *kilometer#QUANTITY*}

\fr{MODE} is also used for focus adverbs:
...
\ex.\dep{Man wusste davon nur_m-mode durch vage *Gerüchte#MESSAGE*}

\ex.\dep{Sie_has-state war selbst_m-mode außer *sich#STATE* vor Angst}

For connective adverbs, see Section~\ref{sec:modifiers}.
...
as in verbal idioms, weather verbs, existential \emph{there}, names, other
fixed expressions, or specialized sublanguages for such things as dates and
times. (Light verbs, on the other hand, are treated with \fr{SCENE}, see
...
\ex.\dep{kurz_m-quantity>>m-noncomp vor *elf#TIME*}

If an argument of the whole predicate syntactically attaches to an
\rl{x-noncomp} dependent, assign it the same frame as the top predicate:

\ex.\dep{Kim_x-causer *kicked#EVENT* Sandy__undergoer 's **ass#EVENT**_x-noncomp}

If you annotate a literal and a figurative meaning, only the literal meaning
need be considered in the annotation of the \rl{x-noncomp} dependent:

\ex.\dep{wenn man es **fertig#SCENE-DEINIT**_target-scene>>x-noncomp *brachte#SCENE-INIT* , dem Televisor ein Schnippchen zu schlagen_participant>>target-scene__initial-scene}

In other cases, frame the \rl{x-noncomp} dependent as \fr{NONCOMP}:

\ex.\dep{Kim_destroyed *kicked#DESTRUCTION* the **bucket#NONCOMP**_x-noncomp}

Bound pronouns that are part of multiword expressions (e.g., inherently
reflexive verbs) should get the same role as their antecedent:

\ex.\dep{Kim_x-experiencer *erinnert#MESSAGE* sich_x-experiencer nicht_m-mode}

\ex.\dep{I_x-sender *address#MESSAGE-INIT* myself_x-sender to you_x-experiencer}

\ex.\dep{I_has-state *jumped#STATE-CHANGE* to my__has-state **feet#STATE-CHANGE**_x-noncomp}

\ex.\dep{I_accompanied *had#ACCOMPANIMENT* with me_accompanied neither a mechanic_accompanier nor any passengers_accompanier}
...
\ex.\dep{Kim_whole 's *leg#PART-WHOLE*}
...
\ex.\dep{Sie sang so_m-quantity *schön#QUALITY* , dass wir weinten}
...
Note that every dependent of the predicate that is necessarily a participant in
the embedded \rl{scene} should be labeled \rl{participant}, even when they
participate in different roles:

\ex.\dep{Kim_participant__x-causer *gave#SCENE* Sandy_participant__hit a **kick#HITTING**_scene}

\ex.\dep{Kim_participant__target-possessor *bekam#SCENE-INIT* Sandy_participant__possessed zu **fassen#POSSESSION-INIT**_target-scene}

\ex.\dep{Winston_participant__participant___x-experiencer *machte#SCENE-INIT* nie_m-time den **Versuch#SCENE-INIT**_target-scene , das___topic zu ***prüfen#MESSAGE-INIT***__target-scene}

On the other hand, \fr{SCENE} predicates may have arguments that are not
members of the embedded \rl{scene}, such as \rl{x-causer}:
...
\ex.\dep{Sie_x-sender *sang#MESSAGE-INIT* so_m-quality , dass wir weinten_m-result}
...
\ex.\dep{*Nothing#CLASS-NEG* about him_m-whole suggested a child}
...
Senders need not be animate or active:

\ex.\dep{The alarm clock_sender *beeped#SENDING*}

\ex.\dep{Er_x-experiencer *forschte#MESSAGE-INIT* in seinen Kindheitserinnerungen_x-sender nach_topic}

\ex.\dep{Die Polizei_x-experiencer *horchte#MESSAGE-INIT* ihn_x-sender aus_m-noncomp}

\ex.\dep{Kim_x-experiencer had *read#MESSAGE-INIT* that_target-content in a book_x-sender}

\ex.\dep{Miniwahr_identifier , wie_m-identifier es_identified in der Neusprache_m-sender *hieß#IDENTIFICATION*}

Use \fr{SENDING} rather than \fr{MESSAGE} for predicates that cannot take a \rl{topic} argument:

\ex.\dep{Er_x-causer>>sender *schleuderte#LOCATION-DEINIT>>SENDING* eine Flut_has-location>>sent von Gestammel aus sich_initial-location>>x-noncomp heraus}
...
\ex.\dep{Eine Aussöhung_has-condition *bedingt#CONDITION* eine Entschuldigung_condition}
...
\ex.\dep{Kim_x-causer__x-sender *reacted#REACTION* to the allegations_trigger with a **denial#MESSAGE-INIT**_reaction}

Can also be the reaction to an \emph{anticipated} scene:

\ex.\dep{als *stemme#REACTION* er_x-causer sich_x-causer dem Anprall_trigger einer Woge entgegen_m-noncomp}
...
\ex.\dep{Kim_excreter *sneezed#EXCRETION* the napkin_x-resultative__x-has-location off the **table#CLASS**_x-resultative}
...
\newpage\subsection{\emoji{red-triangle-pointed-up} \fr{SUBCLASS}}
\label{sec:SUBCLASS}

\rl{subclass} denotes a subclass of \rl{superclass}.

\ex.\dep{eine *Art#SUBCLASS* Paranoia_superclass}

\ex.\dep{Das_x-subclass ist seine_participant *Art#SCENE* , sich zu bewegen_scene}

\ex.\dep{*birds#CLASS* such as storks_m-subclass}

\ex.\dep{Sie_x-experiencer *lernen#MESSAGE-INIT*__subclass **beispielsweise#SUBCLASS**_m-scene wissenschaftliche Methoden_target-content}
...
\ex.\dep{Die Hassovation_has-time *dauerte#TIME* dreißig Sekunden_time}
...
\ex.\dep{Kim_m-possessor 's *glasses#CLASS*}

\ex.\dep{Kim_m-possessor 's *chair#CLASS*}
...
\ex.
\a.\dep{Kim_has-social-relation war linien_social-relation *treu#SOCIAL-RELATION*} (hypothetical sub-word annotation)
\b.\dep{Kim_has-social-relation>>has-quality war *linientreu#SOCIAL-RELATION>>QUALITY*} (consistent actual annotation)
...
\ex.\dep{Personen_x-experiencer, mit denen_x-experiencer er_x-sender noch nie_m-time ein Wort_x-noncomp *gewechselt#MESSAGE-INIT* hatte}

Relatedly, sometimes a single argument denotes both the initial and the target arg2. In this case, default to \fr{initial-}.

\ex.\dep{Die Programme_initial-scene *wechselten#SCENE-CHANGE* von Tag_m-time zu Tag_m-time}
...
\ex.\dep{Ich glaube nicht, dass ihr_undergoer etwas_event *passiert#EVENT* ist}

\ex.\dep{Sie_has-state war im Gesicht_x-attribute rot_target-state *angelaufen#STATE-INIT*}
...
\ex.\dep{Kim_participant__undergoer *nahm#SCENE* ein **Bad#EVENT**_scene} $\rightarrow$ \dep{Kims_undergoer *Bad#EVENT*}

\ex.\dep{Ihm_x-experiencer *kam#MESSAGE-INIT* nie_m-time in den Sinn_x-noncomp , dass das nötig_target-content sein könnte} $\nrightarrow$ \dep{sein_x-experiencer *Sinn#MESSAGE*, dass das nötig_content sein könnte}
...
\ex.
\a.\dep{Er_x-experiencer konnte ihn_topic nie_m-time *sehen#MESSAGE* , ohne sich aufzuregen_m-accompanier}
\b.Rather than: \dep{Er_x-experiencer konnte ihn_topic nie_m-time *sehen#MESSAGE-INIT* , ohne sich aufzuregen_m-accompanier}
...
\ex. \dep{dass man_participant *gezwungen#SCENE-INIT* wurde mitzumachen_target-scene}
...
\ex.\dep{This_part was *part#PART-WHOLE* of the Hate Week_whole}

\ex.\dep{Kim_social-relation is my_has-social-relation *friend#SOCIAL-RELATION*}

\ex.\dep{Kim_x-sender was my_topic *critic#MESSAGE-INIT*}
...
\ex.\dep{Unwissenheit_x-has-class ist *Stärke#QUALITY*}
...
\ex.\dep{Parallelen_x-noncomp *ziehen#COMPARISON* zwischen Spielen_reference und dem täglichen Leben_compared}

\ex.\dep{*Gegensatz#COMPARISON* zwischen seinen höflichen Manieren_compared und seinem Preisboxertypus_reference}
...
\ex.\dep{a *question#MESSAGE* of life_topic or death_topic}
...
\ex.\dep{Sein Gesicht_has-state ist *rot#STATE*}

\ex.\dep{Sein Gesicht_has-state ist *gerötet#STATE-INIT*}
